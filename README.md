...
---
## 🔧 프로젝트 종료 후 개선 작업
> 프로젝트 완료 후, 스케줄러 구조의 잠재적 성능 문제를 직접 발견하고
> 별도로 개선을 시도한 내용입니다. 
### 3. 100만 건 대용량 데이터 처리 성능 최적화

**[상황]**  
10분 주기 스케줄러로 100만 건의 더미 데이터를 DB에 저장하는 과정에서 실제 실행 시간이 20분을 초과, 스케줄러가 겹치는 구조적 문제가 발생했습니다.
...
| 방안 | 내용 |
|------|------|
| **인덱싱 전략** | 자주 조회되는 컬럼에 Index 설정 → 조회 효율 개선 |
| **배치 처리** | 일정 단위로 데이터를 나누어 처리 → 트랜잭션 부하 분산 |
